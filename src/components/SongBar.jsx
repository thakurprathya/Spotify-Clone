...
const SongBar = ({ song, index, artistId, isPlaying, activeSong, handlePauseClick, handlePlayClick }) => {
  ...
  return (
    <div className={`w-full flex flex-row items-center hover:bg-[#4c426e] ${currentTitle === currentSong ? 'bg-[#4c426e]' : 'bg-transparent'} 
      ...
      <div className="flex-1 flex flex-row justify-between items-center">
        ...
        <div className="flex-1 flex flex-col justify-center mx-3">
          {!artistId ? (
            <Link to={`/songs/${song?.id}`}><p className="text-xl font-bold text-white">{song?.attributes?.name}</p></Link>
          ) : (
            <p className="text-xl font-bold text-white">{song?.attributes?.name}</p>
          )}
          <p className="text-base text-gray-300 mt-1">{artistId ? song?.attributes?.artistName : song?.subtitle}</p>
        </div>
    </div>
    <PlayPause
        isPlaying={isPlaying}
