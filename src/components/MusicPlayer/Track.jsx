...
const Track = ({ isPlaying, isActive, activeSong }) => {
  ...
  return (
    <div className="flex-1 flex items-center justify-start w-[70%] md:w-[30%]">
      <div className={`${isPlaying && isActive ? 'animate-[spin_3s_linear_infinite]' : ''} hidden sm:block h-16 w-16 mr-4`}>
        <img src={currentImage} alt="cover art" className="rounded-full" />
      </div>
      <div className="w-[50%]">
        <p className="truncate text-white font-bold text-lg">{currentTitle || 'No active Song' }</p>
        <p className="truncate text-gray-300">{currentSubtitle || 'No active Song'}</p>
      </div>
    </div>
  );
};
